...
# load sql db
DATABASE_FILE="./database/data-"$user_input".db"
# get all tables within the db
TABLES=$(sqlite3 $DATABASE_FILE ".tables")

# drop old table
sqlite3 $DATABASE_FILE "DROP TABLE IF EXISTS new_table;"

# create table
sqlite3 $DATABASE_FILE """CREATE TABLE new_table AS SELECT COALESCE(dns.[trans_id], NULL) AS [trans_id],
